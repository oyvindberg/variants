...
object GenTransformer extends (AdtMetadata => Defn) {
  ...
  override def apply(metadata: AdtMetadata): Defn = {
    ...
    val newTparams: Seq[Type.Param] =
      ScopeTParam +: tparamsNoVariance

    val newTparamNames: Seq[Type.Name] =
      Scope +: tparamsNoVariance.map(tp => Type.Name(tp.name.value))
    ...
    val methods: Seq[Defn.Def] =
      (metadata.branches map createBranchVisitDef(metadata.inheritance)) ++
        (metadata.leafs map createLeafVisitDef(metadata)) ++
        createCombineMethods(newTparamNames, entryDefs, TransformerType) ++
        entryDefs
  ...
  private def createCombineMethods(newTparams: Seq[Type.Name], entryDefs: Seq[Defn.Def], TransformerType: Type.Name) = {
    val This = t"$TransformerType[..$newTparams]"

    val one = q"final def >>(that: $This): $This = combine(that)"
    val two = q"""
      final def combine(that: $This): $This = {
        val self = this
        new ${type2ctor(TransformerType)}[..$newTparams]{
          ..${entryDefs.map(d =>
      d.copy(mods = d.mods :+ Mod.Override(), body = q"that.${d.name}($scope)(self.${d.name}($scope)($first))"))}
        }
      }"""
  ...
  def createLeafVisitDef(metadata: AdtMetadata)(leaf: Defn): Defn.Def =
    leaf match {
      ...
      case Defn.Class(_, tpe, tparams: Seq[Type.Param], Ctor.Primary(_, _, pss), _) =>
        ...
        val baseCase: PartialFunction[Type, Term => Term] = {
          case Type.Apply(tname @ Type.Name(value), _) if metadata.localNames(value) =>
            withTerm(term => q"${visitMethodName(tname)}($childScope)($term)")
          case tname @ Type.Name(value) if metadata.localNames(value) =>
            withTerm(term => q"${visitMethodName(tname)}($childScope)($term)")
        }

        val newInstance = DeriveNewInstance(metadata.externalFunctors, baseCase,second, tpe, pss)
        ...
        final def ${visitMethodName(tpe)}($scope: $Scope)($first: $Tpe): $Tpe = {
          ...
          val ${term2pat(third)}: $Tpe = $newInstance
          $third
        }"""
    }
